...
namespace TrafficControlTest.Module.Log
{
	...
	/// - 每當 IVehicleControlManager 發生 ItemAdded, ItemUpdated 事件時，使用 IEventRecorder 將資料記錄至資料庫 (供快速查詢歷史任務)
	/// - 每當 IAutomaticDoorControlManager 發生 ItemAdded, ItemUpdated 事件時，使用 IEventRecorder 將資料記錄至資料庫 (供快速查詢歷史任務)
	...
	public interface IImportantEventRecorder : ISystemWithLoopTask
	{
		...
		void Set(IEventRecorder EventRecorder, IVehicleInfoManager VehicleInfoManager, IMissionStateManager MissionStateManager, IVehicleControlManager VehicleControlManager, IAutomaticDoorControlManager AutomaticDoorControlManager, IHostCommunicator HostCommunicator);
	}
}
